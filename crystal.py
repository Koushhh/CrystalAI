...
import speech_recognition as sr                          # Koushal's Project
import datetime
import os
import cv2
import requests
...
def wish():
    ...
    while True:
       ...
       elif "system status" in query or "system peformance" in query:
           battery =psutil.sensors_battery()
           per = battery.percent
           plug = battery.power_plugged 
           cpu = psutil.cpu_percent(interval=1)
       ...
       elif "i will call you back" in query or "wait in background" in query or "wait for me in background" in query or "i call you back" in query or "stay in background" in query:
